use super::*;

pub(in crate::native_bridge::document) fn install_detached_character_data_instance_properties(
    scope: &mut v8::PinScope<'_, '_>,
    object: v8::Local<'_, v8::Object>,
) {
    install_detached_node_core_instance_properties(scope, object);
    install_detached_non_document_type_child_node_instance_properties(scope, object);
}

pub(in crate::native_bridge::document) fn install_detached_processing_instruction_instance_properties(
    scope: &mut v8::PinScope<'_, '_>,
    object: v8::Local<'_, v8::Object>,
) {
    install_detached_character_data_instance_properties(scope, object);
}
