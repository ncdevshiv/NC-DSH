...
promise_test(async t => {
  ...
  // Chromium currently marks the upstream file Timeout because this subtest
  // uses DedicatedWorker-style self.postMessage / worker.onmessage. Keep the
  // intended blob URL base assertion, but use the SharedWorker MessagePort.
  const blob_contents = `
onconnect = e => {
...
port.postMessage(constructedRequest);
};
`;
  const blob = new Blob([blob_contents]);
  const url = URL.createObjectURL(blob);
