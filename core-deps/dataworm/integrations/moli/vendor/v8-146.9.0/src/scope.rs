...
///
/// Entering a `ContextScope` selects the callback's relevant Realm; it does
/// not preserve the incumbent Realm captured when a Web IDL callback was
/// converted. Callback invocation needs both scopes.
///
/// V8 links this object into an isolate-owned stack by its address. It must
/// therefore be pinned before construction and remain at that address until
/// destruction. Use `std::pin::pin!(BackupIncumbentScope::new(context))`, then
/// call `ScopeStorage::init()` before invoking JavaScript.
...
impl ScopeInit for BackupIncumbentScope<'_> {
  fn init_stack(
    storage: Pin<&mut ScopeStorage<Self>>,
  ) -> Pin<&mut Self> {
    let storage_mut = unsafe { storage.get_unchecked_mut() };
