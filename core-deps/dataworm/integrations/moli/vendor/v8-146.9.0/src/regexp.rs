...
impl RegExp {
  ...
  pub fn exec<'s>(
    ...
    // V8 documents no-match as returning Null even though the C++ API is
    // MaybeLocal<Object>. Callers that care about match/no-match must check
    // result.is_null() before treating the returned handle as an object.
    unsafe {
      scope.cast_local(|sd| {
