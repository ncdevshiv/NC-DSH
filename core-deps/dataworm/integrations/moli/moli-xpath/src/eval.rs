...
pub(crate) enum NameTestComparisonMode {
    /// Namespaces and local names must match exactly.
    XHtml,
    /// Missing namespace information is treated as the HTML namespace and
    /// local names are compared ASCII case-insensitively.
    HtmlElement,
    /// Attribute namespaces still match exactly, but local names on HTML
    /// elements in HTML documents are compared ASCII case-insensitively.
    HtmlAttribute,
}
...
pub(crate) fn element_name_test(
    ...
    let should_compare_namespaces =
        comparison_mode != NameTestComparisonMode::HtmlElement || expected_name.ns != ns!();
    ...
    match comparison_mode {
        NameTestComparisonMode::HtmlElement | NameTestComparisonMode::HtmlAttribute => {
            expected_name
                .local
                .as_ref()
                .eq_ignore_ascii_case(actual_name.local.as_ref())
        }
        NameTestComparisonMode::XHtml => expected_name.local == actual_name.local,
    }
}
        ...
        NodeTest::Name(expected_name) => {
            if let Some(element) = node.as_element() {
                let comparison_mode = if element.is_html_element_in_html_document() {
                    NameTestComparisonMode::HtmlElement
                } else {
                ...
                let is_html_attribute = attribute
                    .as_node()
                    .parent()
                    .and_then(|node| node.as_element())
                    .is_some_and(|element| element.is_html_element_in_html_document());
                let comparison_mode = if is_html_attribute {
                    NameTestComparisonMode::HtmlAttribute
                } else {
                    NameTestComparisonMode::XHtml
                };
                element_name_test(expected_name, actual_name, comparison_mode)
