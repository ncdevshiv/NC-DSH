...
- **Live toggle only** — `setEnabled` flips `disabled` for the running process via `ctx.loader.update`; the change is not automatically persisted to the profile `cordis.patch.yml` and is lost on restart unless the caller also patches that file. Group entries cannot be toggled. A replacement enable leaves the displaced duplicates disabled for this process; nothing about that swap is persisted either.
