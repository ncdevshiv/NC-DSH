...
`session.fork` 将可选事件锚点映射到该锚点处或其后的首个 `turn/end`，使消息操作可包含该消息所在的完整轮次。锚点省略或超过末尾时，选择最后一个已完成轮次；若锚点已在日志中，而其所在轮次仍开放，则返回 `fork-unavailable`，不会向较早位置裁剪。发布后的子会话会先继承源会话的种子历史、cwd、日志中最新的 `ModelSelection` 及谱系，再加入源 Workspace。显式传入 `workspaceId` 会重定向 fork：子会话采用该 Workspace 的路径作为自身 cwd 并加入该 Workspace，而不是跟随源会话——这样无 Workspace 开始的对话可以带着完整历史转入某个 Workspace（种子仍引用源 cwd）。未知 id 返回 `workspace-not-found`；发布后附加失败仍返回 `workspace-attach-failed` 并携带已发布的子会话 id，供客户端对账。[SessionStore fork 决策](../../../.agents/notes/implemented/feature/2026-06-30-session-store-fork-api.md)记录了为何锚点要映射到该 `turn/end`。
