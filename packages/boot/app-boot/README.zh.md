...
| `watchUserPatchesAcrossHmrSwaps(ctx, optionsList)` | 挂载一个要求 `hmr` 的 `user-patch-watch` 插件，并通过 `watchUserPatches` 注册列出的每个文件；Cordis 会针对每个新的 `hmr` 实例重新运行这些注册，因此切换 HMR 条目的设置开关也能保住 patch 监视。调用方必须保证当前就能解析到 `hmr` 服务；首次应用在此处结算，并经调用方的启动错误路径抛出 |
