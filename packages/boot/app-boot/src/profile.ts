 ...
 * `node_modules` the package manager maintains for out-of-tree plugins, while
...
// installation's single cordis instance instead of a duplicate. Bun reads its
// install settings from bunfig.toml, not package.json.
 ...
 * install settings out-of-tree plugins need. Existing files are never touched,
 ...
 * resolves without the package manager ever installing it — the exact "bundles come from the
 ...
 * `existsSync` follows the symlinks the isolated store layout uses.
 */
