...
- **只观测交接启动**：平台 opener 接受 spawn 后即结束观察；之后的浏览器退出不会上报，已打印 URL 仍是手动访问的回退路径。
