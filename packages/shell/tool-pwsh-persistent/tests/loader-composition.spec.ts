...
describe.skipIf(!hasPwsh)('persistent pwsh through a real cordis.yml Loader composition', () => {
  it('preserves cwd and environment across calls', async () => {
    ...
    // Windows may surface the 8.3 alias of a temp ancestor; compare the two
    // directories by their resolved identity instead of their spelling.
    const reported = /cwd=(.*) keep=loader/.exec(observed)?.[1]
    expect(reported).toBeDefined()
    expect(await realpath(reported as string)).toBe(await realpath(join(root, 'nested')))
    expect(observed).toContain('keep=loader')
    ...
    // Same 8.3-alias caveat as above: compare resolved identity.
    const afterExit = text(await execute('after-exit', 'Write-Output "$PWD"'))
    expect(await realpath(afterExit)).toBe(await realpath(root))
  }, 60_000)
})
