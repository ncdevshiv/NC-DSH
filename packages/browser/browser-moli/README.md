...
- One session = one serve process: state isolation between launches, one lifecycle controller per session owning child process plus WebSocket, torn down by `close()` from any path. Closing the connection settles every blocked command and event waiter immediately, so `close()` never waits out a deadline timer. Serve processes left behind by a crash or hard exit are force-killed during Node's synchronous exit phase.
...
- Every operation honors the caller's `AbortSignal` through the CDP layer: cancellation rejects as `BROWSER_ABORTED` promptly, including during navigation and startup discovery.
...
| `cdpTimeoutMs` | `30_000` | Budget for one CDP command or event wait outside navigation (`Runtime.evaluate`, screenshots). |
| `maxContentChars` | `100_000` | Character cap on returned page text content. |
| `settleMs` | `150` | Settle delay after a DOM interaction before the state read. |
