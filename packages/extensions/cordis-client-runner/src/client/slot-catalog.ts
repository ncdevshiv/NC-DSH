...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    occupants: [
      'client-ui-layout SpectrumAppFrame',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-notifications NotificationsBell id \'notifications-bell\'',
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table Record<SidebarSectionKey, object>, already taken: code, home, team, work',
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-home-section HomeSection key \'home\'',
      'client-ui-team-section TeamSection key \'team\'',
      'client-ui-work-section WorkSection key \'work\'',
      'client-ui-workspace WorkspaceBrowser key \'code\'',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidebar.section\', () => ctx.slots.register(\n      { name: \'sidebar.section\', key: \'<one key the owner dispatches>\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-sidebar/src/client/contract/slots.ts:40',
  },
  {
    key: 'sidebar.section.directoryFlow',
    kind: 'single',
    scope: 'root',
    summary: 'Directory-flow hole under the sidebar section browser (declared by the WorkspaceBrowser entry).',
    doc: 'Directory-flow hole under the sidebar section browser (declared by the WorkspaceBrowser entry).',
    registerOptions: [],
    ownerProps: [
      '/**\n * Owner share of the directory-flow holes: the complete conversation between\n * the trigger surface and the picking interaction. The occupant reads `open`\n * to run/render its interaction and reports exactly one outcome per open.\n */\nexport interface DirectoryFlowOwnerProps {\n  /** True while a picking interaction is requested; flipping back to false withdraws the request. */\n  open: boolean\n  /** True while the owner adopts a picked path (`createWorkspace` in flight); occupants disable their commit affordances. */\n  busy: boolean\n  /** The operator picked a directory (absolute host path); the owner adopts it. */\n  onPicked: (path: string) => void\n  /** The operator dismissed the interaction; the owner just closes the flow. */\n  onCancel: () => void\n  /** The interaction itself failed (chooser missing, listing denied); the owner shows its error surface. */\n  onError: (message: string) => void\n}',
    ],
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'sidebar.section\' (client-ui-workspace), so it exists while that entry is mounted',
    occupants: [
      'client-ui-directory-picker-browse BrowseDirectoryFlow',
      'client-ui-directory-picker-native NativeDirectoryFlow',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidebar.section.directoryFlow\', () => ctx.slots.register(\n      { name: \'sidebar.section.directoryFlow\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
