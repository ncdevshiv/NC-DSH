...
function targetOf(row: ProviderRow): EditorTarget {
  ...
  const credentialRef = (() => {
    if (row.apiKeyEnv === undefined || row.credential?.configured !== true || !row.credential.writable) return undefined
    if (row.entry.settingsPath.length === 0) return row.apiKeyEnv
    return row.apiKeyEnv === managedRef ? managedRef : undefined
  })()
  return {
