...
export const en = {
  ...
  resetTitle: 'Reset {provider} to defaults?',
  resetRestoresBase: 'Resetting {provider} removes any customized settings and restores the provider defaults. Any stored API key is managed elsewhere and will be kept.',
  resetRestoresBaseWithCredential: 'Resetting {provider} restores the provider defaults and removes the stored API key.',
  resetConfirm: 'Reset {provider}',
  resetting: 'Resetting {provider}…',
  add: 'Add provider',
  provider: 'Provider',
  close: 'Close',
  cancel: 'Cancel',
  ...
  modelVision: 'Vision',
  modelVisionHint: 'Image input. Checked declares image support; unchecked declares text-only. Untouched rows keep the provider default.',
  addModel: 'Add model',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  reset: '重置',
  resetProvider: '重置 {provider}',
  resetTitle: '将 {provider} 恢复为默认值？',
  resetRestoresBase: '重置 {provider} 会移除自定义设置并恢复提供方默认值；其使用的凭证（如有）由其他位置管理，将会保留。',
  resetRestoresBaseWithCredential: '重置 {provider} 会恢复提供方默认值并移除存储的 API 密钥。',
  resetConfirm: '重置 {provider}',
  resetting: '正在重置 {provider}…',
  add: '添加提供方',
  provider: '提供方',
  close: '关闭',
  cancel: '取消',
  ...
  modelAdvanced: '容量',
  modelVision: '视觉',
  modelVisionHint: '图片输入。勾选表示支持图像；不勾选表示仅文本。未改动过的行沿用提供方默认值。',
  addModel: '添加模型',
  removeModel: '删除模型',
