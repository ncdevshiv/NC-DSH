...
describe('running and lock semantics', () => {
  ...
  it('the inert textarea is disabled and does not open the Workspace picker', () => {
    const { view, textarea } = bench({
      inert: true,
    ...
    expect(card.getAttribute('class')).not.toContain('cardWorkspaceTrigger')
    expect(card.onclick).toBeNull()
    fireEvent.click(card)
    fireEvent.click(textarea)
    fireEvent.keyDown(textarea, { key: 'Enter' })
    fireEvent.keyDown(textarea, { key: ' ' })
    // No picker callback exists — the test proves the trigger is gone.
    expect(textarea.disabled).toBe(true)
  })
