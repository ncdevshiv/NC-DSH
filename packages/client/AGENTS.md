...
[docs/web-styling.md](../../docs/web-styling.md) is authoritative. Shared `--dsw-*` tokens and global sheets live in `ui-theme/src/styles/`; interactive atoms render Adobe React Spectrum through `ui-primitives` (feature packages never import `@adobe/react-spectrum` directly), while feature styling consumes semantic aliases through CSS Modules and `clsx`, with no literal colors or Tailwind. Product copy is Chinese; code comments are English.
