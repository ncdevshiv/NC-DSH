...
应用经由 `SpectrumSurface` 挂载 Adobe React Spectrum 的 Provider（ui-layout 用它包裹自己的 frame 注册），因此任何动态插件包都不导入该库，也不为它注册模块表行。交互原子构建在 React Aria 钩子层（`useButton` 等）之上：本包拥有渲染出的元素，所有原生属性与处理器原样保留，钩子提供按压语义、焦点接线与键盘激活。DeepSeek 主题展开原生 `defaultTheme`，并把语义变量核心重写为指向 `--dsw-*` 的引用（`spectrum-vars.module.css`），ui-theme token 表因此仍是唯一的颜色权威：两个配色槽位携带相同的覆盖值，明暗切换跟随 `body[data-ds-dark-theme]`。依据、vmForks 测试泳道要求与延后的移植项（Input/Menu/Modal/Tooltip/Toast）：[Spectrum 移植基础 note](../../../.agents/notes/proposed/architecture/2026-08-22-spectrum-port-foundation.md)。
