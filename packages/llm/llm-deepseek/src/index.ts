 ...
 *
 * The whole `llm-deepseek` section is the profile — an empty section keeps the
 * adapter on its defaults (the public endpoint, the `DEEPSEEK_API_KEY`
 * reference), so an exported key works without any stored configuration and the
 * route stays registered even with no user-layer profile.
...
/** DeepSeek provider profile shape — same as {@link Config}. */
export type DeepSeekProviderProfile = Config
...
/** Validate one {@link Config} into connection facts. */
function resolveProfile(profile: Config, environment?: LaunchEnvironmentSnapshot): ResolvedDeepSeekOptions {
