...
- pi-ai reports failures as in-stream error events; these map to `finish {kind:'error'|'aborted', failure}` chunks. Provider-specific error text distinguishes terminal `QUOTA` from transient `RATE_LIMIT`, while text and usage signals evaluated against the resolved model's context window normalize overflow to `CONTEXT_WINDOW_EXCEEDED`. Transient transport wordings — pre-terminal truncation, socket and DNS failures, gateway cutoffs, explicit provider retry guidance, gRPC throttling — map to `TRANSPORT`, `SERVER`, or `RATE_LIMIT`, tracking pi-ai's own transient-error pattern table because adapter calls pin its internal retries off (`maxRetries: 0`). Each attempt's HTTP status and provider request id (`x-request-id` first, then `x-deepseek-request-id`) are captured at pi-ai's response-boundary hook, which fires before body consumption, so a mid-body truncation's error finish and an idle-timeout throw carry those facts; caller aborts stay bare. A terminal `stop` whose message carries no content blocks maps to a `finish {kind:'error'}` with code `EMPTY_RESPONSE` (retried by default policy) instead of a successful empty message.
...
- **Provider HTTP status comes only from the boundary capture** — pi-ai error events themselves carry no status; the adapter attaches the status and request id it observed at pi-ai's `onResponse` hook (fired by every shipped assistant protocol). A failure thrown before any response arrives — DNS refusal, connection reset before headers — exposes codes but no status.
