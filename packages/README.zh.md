...
| [`terminal/`](terminal/README.md) | 持久 PTY 能力系列：限定所有者范围的会话、本地实现、面向模型的工具 | 产品：稳定 API |
...
| [`context/`](context/README.md) | 模型可见请求上下文：workspace 指令与时间上下文 | 产品：稳定 API |
...
| [`browser/`](browser/README.md) | 浏览器自动化能力系列：seam、moli CDP 提供方实现和面向模型的浏览器工具 | 产品：稳定 API |
| [`mcp/`](mcp/README.md) | MCP 客户端桥：把外部服务器工具注册到 `ctx.tools` | 产品：稳定 API |
...
| [`guard/`](guard/README.md) | 循环卫生守卫：重复调用提醒 + `tools/execute` 截止时间强制执行器 | 产品：稳定 API |
...
| [`session/`](session/README.md) | 持久会话数据平面：持久化 + JSONL/SQLite 后端、投影、基于日志的标题、会话上报 | 产品：稳定 API |
| [`session-query/`](session-query/README.md) | 会话检索系列：逻辑语料库、有界读取、血缘、事件关系、语义过滤、SQLite 全文搜索 | 产品：稳定 API |
...
| [`util/`](util/README.md) | 组间共享的零依赖工具（`Branded<B>`、home 路径、超时、留存） | 支持：小型、稳定、无 harness 依赖 |

新包加入现有组；新组更新其 README 和此表。

## 依赖

依赖图由工具生成：[docs/module-graph.md](../docs/module-graph.md)（`bun run gen-module-graph`，CI 中有新鲜度门禁）。
