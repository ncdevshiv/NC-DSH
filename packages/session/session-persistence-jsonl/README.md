...
- **Foreign-writer refusal.** Every append verifies the file's current byte size against what this backend instance last wrote or revision-stably read before writing; a mismatch rejects the append loudly instead of interleaving duplicate sequence numbers that make the log unreadable. The baseline comes from materialization, each committed append, repair truncation, and adoption reads; a fresh instance seeds it from its adoption read, so normal resume never false-positives.
...
- **One live writer per session, enforced at append time** — every durable append verifies the log's byte size against this backend instance's last-written or adoption-read baseline and refuses a diverged file loudly; concurrent writers therefore fail instead of interleaving duplicate sequence numbers. The residual gap is the window before an instance's first read or write of a session (it cannot yet know a baseline), and other `SessionPersistence` backends enforce their own equivalents (the SQLite backend validates stored seq on every mutation).
