...
`landlock-run/` and its packages belong to the repository's root bun workspace and `bun.lock`. Harness consumers use the current workspace entry package during development and CI, so a launcher contract change and its consumer update can land and be tested together.
