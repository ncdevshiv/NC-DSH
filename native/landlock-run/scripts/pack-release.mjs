 ...
 * them), and write `publish-order.txt` next to them. The entry pack produces
 * the EXACT bytes a publish would upload and runs each package's
...
for (const dir of dirs) {
  ...
  // Platform packages are packed with npm: an installer's pack path may
  // normalize file modes and strip the executable bit, which would ship a
  // launcher no consumer can spawn; npm pack preserves it. Platform packages
  // have no dependencies by construction, so they need no workspace-protocol
  // conversion. Entry packages need that conversion and carry no executables,
  // so they keep the installer's own pack.
