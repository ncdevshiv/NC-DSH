...
Platform tarballs are produced by `npm pack`, entry tarballs by the workspace installer's own pack — deliberately split: an installer's pack path may normalize file modes and strip the executable bit, which would ship a launcher no consumer can spawn, while platform packages have no dependencies and so need no workspace-protocol conversion; entry packages need that conversion and carry no executables. `scripts/pack-release.mjs` encodes the split — never hand-pack a platform package with an installer whose mode handling is unproven for your checkout.
