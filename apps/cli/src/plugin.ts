...
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  try {
    ...
    return false // the install reported success yet the package is unresolvable — treat as plain
  }
...
/**
 * Reconcile `dsh.profile.bundles` against the installed state: the install has
 ...
 * The install runs with cwd = the profile directory, so a bare `.` or
 * `../plugin` (or their `file:`/`link:` forms) would silently resolve inside
 * the profile — `add .` from a plugin checkout would self-link the profile.
 * Absolute specs, registry names, and every other argument pass through
 * untouched.
 * @param argument - one forwarded argument, verbatim from argv.
...
function anchorPathSpec(argument: string, cwd: string): string {
  ...
  // A bare path stays bare and a prefixed spec keeps its prefix: the
  // installer's link-vs-copy semantics differ between `file:` and a plain
  // directory path, and the anchor must not change which one the user asked for.
  const prefix = match.groups.prefix ?? ''
  return `${prefix}${resolve(cwd, match.groups.path)}`
}

/**
 * Map one forwarded argument vector onto the bun CLI. The common subcommands
 * line up (`add`, `remove`, `install`, `update`); `why` is the one divergence,
 * living under `bun pm why`.
 * @param args - forwarded arguments with relative path specs anchored to the invoking directory.
 * @returns the argument vector to spawn.
 */
function bunArgs(args: readonly string[]): string[] {
  if (args[0] === 'why') return ['pm', 'why', ...args.slice(1)]
  return [...args]
}
 ...
 * @param args - forwarded arguments with relative path specs anchored to the invoking directory.
 * @returns the forwarded command's exit code.
 */
export function runPlugin(profile: string, args: readonly string[]): number {
  ...
  // bun is a standalone binary, so every platform spawns it directly — no
  // shell, and no .cmd shim for Windows spawn to refuse.
  const result = spawnSync('bun', bunArgs(args.map(argument => anchorPathSpec(argument, process.cwd()))), {
    cwd: dir,
    stdio: 'inherit',
  })
  if (result.error !== undefined) {
    ...
    if (code === 'ENOENT') {
      process.stderr.write(`${NAME}: bun not found on PATH — install bun (https://bun.sh) to manage profile plugins\n`)
      return 127
    }
    throw result.error
  }
  ...
  if (exitCode === 0) {
    ...
    // The installer's own diagnostics name its config files without saying
    // WHICH one; the profile owns them, and the commonest failure here is a
    // git dependency whose prepare (build) script the installer blocks until
    // the package is trusted.
    process.stderr.write(`${NAME}: install failed in profile directory ${dir}\n`)
    if (args.some(argument => /^git\+|^github:|\.git(?:#|$)/.test(argument))) {
      process.stderr.write(
        `${NAME}: git-hosted plugins build on install via their prepare script, which bun blocks until trusted — `
        + `add the package name bun printed above to trustedDependencies in ${join(dir, 'package.json')}, then re-run\n`,
      )
    }
  }
  return exitCode
