...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    ...
    // checkout — bun runs with its cwd at the profile directory, so an
    // un-anchored `.` would self-link the profile.
  ...
  it('activates a dependency that gained dsh.bundle in a later update', async () => {
    ...
    // invocation, so `update` (not only `add`) activates a package whose newer
    // version declares dsh.bundle. Simulated without a registry: hand-place
    // the installed package, flip its manifest, and run a benign read-only
    // bun command.
