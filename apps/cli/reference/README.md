...
Git-hosted plugins that ship sources build during install through their `prepare` script, which bun runs only for dependencies the profile trusts: the first `add` fails with bun's lifecycle-script notice naming the package (and a dsh pointer at the profile manifest's `trustedDependencies`); add the package name there and re-run. Installing a built tarball or a local checkout needs no allowance.
