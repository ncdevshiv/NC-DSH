...
1. Require a clean worktree, record its exact commit with `git rev-parse HEAD`, then build that recorded tree — here, `bun run build && bun run build:web`. A GIF recorded against another commit's build misattributes the evidence.
