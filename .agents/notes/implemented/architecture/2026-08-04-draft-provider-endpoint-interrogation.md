...
> Supersedes-in-place note: the mechanism now lives on the single adapter — `ctx.llm.registerModelDiscovery('llm-ai-sdk', …)` backed by the sidecar's `model.discover` ([model discovery and route dialects](../feature/2026-08-26-model-discovery-and-route-dialects.md)). The decision, alternatives, and adopt-don't-write contract below are unchanged; the pi-ai implementation this note originally described is deleted.
...
`dsh-llm-ai-sdk` implements the wire path through the sidecar's `model.discover`: the child builds a transient provider for one listing call, mapping the draft's explicit `api` (else the route profile, else the route-id table) onto the native Anthropic and Gemini adapters or the OpenAI-compatible family. An unknown dialect fails loud as a typed configuration error before any network call — a wrong guess at a response shape is never indistinguishable from a provider with no models — and an OpenAI-compatible interrogation without an endpoint is refused for the same reason. The route's stored credential is resolved by the plugin's own per-request resolver and only when the draft carries no typed key, so a catalog route answers without touching credentials or the network at all.
...
`packages/llm/llm/tests/topology.spec.ts` covers the registry: one offer per namespace, disposal with the fiber, normalization that drops duplicate and unusable ids without inventing capacities, and the `NO_DISCOVERY`/`INVALID_DISCOVERY` refusals. `packages/llm/llm-ai-sdk/tests/adapter.spec.ts` drives the probe against a scripted sidecar — endpoint-reported rows, the missing-endpoint contract refusal, and the lifecycle that kills a failed generation's child — and `tests/loader-composition.spec.ts` answers a known route from its advisory catalog without any sidecar present; `tests/adapter.e2e.ts` runs discovery through the real release binary against a live OpenAI-compatible listing. `packages/host/apiproxy/tests/api-proxy-config.spec.ts` covers the RPC over a real proxy: the draft reaching its namespace whole, absent fields staying absent, no namespace or credential being written, and a failure surfacing as `model-discovery-failed` with the credential absent from the serialized error.
