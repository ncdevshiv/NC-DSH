...
flowchart LR
  ...
  svc_notifications["ctx.notifications<br/>Dismissible notification seam"]
  pkg_sidecar_updates["sidecar-updates"]
  ...
  svc_sidecarUpdates["ctx.sidecarUpdates<br/>AI sidecar auto-update pipeline"]
  ...
  pkg_browser["browser"]
  svc_browser["ctx.browser<br/>Browser automation provider registry"]
  ...
  pkg_sidecar_updates --> svc_sidecarUpdates
  ...
  svc_attachments --> pkg_llm_ai_sdk
  ...
  svc_credentials --> pkg_llm_ai_sdk
  ...
  svc_notifications --> pkg_apiproxy
  svc_notifications --> pkg_sidecar_updates
  ...
  svc_sidecarUpdates --> pkg_apiproxy
...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.attachments` | `seam` | [`attachment`](../packages/attachment/attachment) | [`attachment-local`](../packages/attachment/attachment-local) | `host-runtime`, [`llm-ai-sdk`](../packages/llm/llm-ai-sdk) | - | 宿主会在会话事件之前提交已接受的图片；提供方适配器将已授权的持久引用解析为提供方原生内容。 |
| `ctx.llm` | `seam` | [`llm`](../packages/llm/llm) | [`llm-ai-sdk`](../packages/llm/llm-ai-sdk), [`llm-replay`](../packages/test-support/llm-replay) | [`agent-loop`](../packages/core/agent-loop), [`compaction-basic`](../packages/compaction/compaction-basic) | - | 适配器注册提供方实现；agent loop（智能体循环）与压缩功能调用提供方无关的流服务。 |
...
| `ctx.notifications` | `seam` | [`notifications`](../packages/host/notifications) | - | [`sidecar-updates`](../packages/sdk/sidecar-updates), `apiproxy` | - | 插件在稳定 id 下发布用户可见通知；该缝拥有已读、已关闭和已删除状态、一份 harness-home 文档，以及由 sidecar 更新器与 Web 网关消费的 notifications/* 变更。 |
...
| `ctx.sidecarUpdates` | `core` | [`sidecar-updates`](../packages/sdk/sidecar-updates) | - | `apiproxy` | - | 轮询 GitHub 仓库的 ai-sidecar 发布，按 SHA256SUMS 校验，按 tag 安装，原子性地重定向 current.json，并发出通过通知缝呈现的 sidecar-updates/status 快照。 |
...
| `ctx.settings` | `seam` | [`settings`](../packages/settings/settings) | [`settings-file`](../packages/settings/settings-file) | [`llm-ai-sdk`](../packages/llm/llm-ai-sdk), `apiproxy` | - | 插件注册命名空间 schema 并解析分层值；提供方存储原始文档。LLM（大语言模型）适配器在用户分区下将其入口配置注册为组合基础；Web 网关提供经过脱敏的分层描述符，并写入用户层。 |
| `ctx.credentials` | `seam` | [`credentials`](../packages/credentials/credentials) | [`credentials-local`](../packages/credentials/credentials-local) | [`llm-ai-sdk`](../packages/llm/llm-ai-sdk), `apiproxy` | - | 配置携带对机密信息的引用；提供方拥有实际值。消费方按操作解析，因此轮换后的凭据会在紧接着的下一次请求中生效；Web 网关提供不含实际值的视图和只写存储。 |
...
| `ctx.web` | `seam` | [`web`](../packages/web/web) | [`web-search-exa`](../packages/web/web-search-exa), [`web-search-perplexity`](../packages/web/web-search-perplexity), [`web-search-deepseek`](../packages/web/web-search-deepseek), [`web-search-searxng`](../packages/web/web-search-searxng), [`web-fetch-http`](../packages/web/web-fetch-http), [`web-fetch-moli`](../packages/web/web-fetch-moli) | [`tool-web`](../packages/web/tool-web) | - | 搜索和抓取提供方注册到同一个 ctx.web seam；tool-web 负责稳定的面向模型名称。 |
| `ctx.browser` | `seam` | [`browser`](../packages/browser/browser) | [`browser-moli`](../packages/browser/browser-moli) | [`tool-browser`](../packages/browser/tool-browser) | - | 浏览器提供方注册到同一个 ctx.browser seam 并启动串行会话；tool-browser 负责稳定的面向模型名称。 |
