...
`dsh plugin --profile <name> <args...>` forwards to bun in the profile directory, so ordinary package-management verbs work unchanged. From the directory that contains `hello-plugin`, install the package checkout:
...
- **The author** ships a `prepare` script — bun runs it for trusted dependencies after a git install — that builds the published entry points from source, self-contained: it must not assume dev-only context such as a sibling monorepo checkout. [turtle-ui](https://github.com/deepseek-harness/turtle-ui) is a working example: its `prepare` runs a dedicated tsdown config that transpiles `src/` without project references or type checking.
- **The user** trusts the build. Bun runs an install script only when the consuming manifest lists the dependency under `trustedDependencies`, so the first `add` of an untrusted git dependency fails; `dsh` points at the fix — add the package name to the profile manifest's `trustedDependencies`:
...
Treat that trust as what it is: **permission to execute the package's code on your machine at install time**, outside any sandbox the agent runs under. Only trust packages whose source you have vetted, and pin a commit (`github:you/hello-plugin#<sha>`) so a later push cannot silently change what runs.
