...
describe('CI workflow', () => {
  it('installs dependencies through bun with the lockfile frozen in every job that installs', () => {
    ...
    const installing = Object.entries(workflow.jobs).flatMap(([jobName, job]) => {
      if (!isRecord(job) || !Array.isArray(job.steps)) return []
      const runs = job.steps.filter((step): step is Record<string, unknown> & { run: string } => (
        isRecord(step) && typeof step.run === 'string' && step.run.includes('install --frozen-lockfile')
      ))
      return runs.length === 0 ? [] : [{ jobName, job, runs }]
    })

    expect(installing.length).toBeGreaterThan(0)
    for (const { jobName, job, runs } of installing) {
      if (!Array.isArray(job.steps)) continue
      const setsUpBun = job.steps.some(step => isRecord(step) && typeof step.uses === 'string' && step.uses.startsWith('oven-sh/setup-bun@'))
      expect(setsUpBun, `${jobName} must run oven-sh/setup-bun before installing`).toBe(true)
      for (const run of runs) {
        expect(run.run, `${jobName} must install immutably`).toContain('bun install --frozen-lockfile')
      }
    }
    expect(JSON.stringify(workflow), 'no lane may configure a pnpm cache').not.toContain('cache: pnpm')
  })
...
describe('Python release workflows', () => {
  ...
  it('exposes the native wheel builder to the release caller with normalized versions', () => {
    ...
    expect(JSON.stringify(manylinuxAddon)).toContain('$HOME/.cache/node-gyp:$HOME/.cache/node-gyp:ro')
