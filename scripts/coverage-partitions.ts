...
export interface CoverageCommand {
  ...
  /** Executable to spawn. */
  command: string
  /** Command-line arguments for {@link CoverageCommand.command}. */
  args: string[]
...
export interface CoveragePartitionCoordinatorOptions {
  ...
  partitions: number
  /** Package-manager JavaScript entrypoint or standalone binary path from `npm_execpath`. */
...
/** Remove the package-script argument separator before forwarding Vitest arguments. */
...
export class CoveragePartitionCoordinator {
  ...
  private readonly packageManagerEntrypoint: string
  ...
  public constructor(options: CoveragePartitionCoordinatorOptions) {
    ...
    this.packageManagerEntrypoint = options.packageManagerEntrypoint
  ...
  private partitionCommand(index: number): CoverageCommand {
    ...
    const invocation = vitestInvocation(this.packageManagerEntrypoint, this.root, [
      'run',
      '--coverage',
...
export function vitestInvocation(
  ...
  if (/.exe$/i.test(entrypoint) || userAgent.startsWith('bun/')) {
    ...
    if (!existsSync(launcher)) throw new Error(`coverage partitions: vitest is not installed at ${launcher}.`)
    return { command: launcher, args: [...vitestArgs] }
  }
  return { command: process.execPath, args: [entrypoint, 'exec', 'vitest', ...vitestArgs] }
}

/** Spawn one command without a platform shell. */
