...
// The invocation helper inspects this variable; pin a node-backed manager so these
// tests exercise the JavaScript-entrypoint branch regardless of the outer runner.
...
describe('coverage partition coordinator', () => {
  it('runs every single-worker partition before one merged threshold check', async () => {
    ...
    const coordinator = new CoveragePartitionCoordinator({
      root,
      partitions: 3,
      packageManagerEntrypoint: '/pnpm.cjs',
