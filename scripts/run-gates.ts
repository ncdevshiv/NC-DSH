...
function scriptGate(id: string, script: string, options: Partial<Gate> = {}): Gate {
  return {
    id,
    label: options.label ?? script,
    displayCommand: `bun run ${script}`,
    ...packageManagerInvocation(['run', script]),
    ...options,
  }
}
...
function ciBuildGate(id = 'build', options: Partial<Gate> = {}): Gate {
  return scriptGate(id, 'build', {
    ...options,
    env: { ...options.env, [CLIENT_BUILD_PROFILE_SELECTOR]: 'official' },
  })
}

function execGate(id: string, args: string[], options: Partial<Gate> = {}): Gate {
  return {
    id,
    label: options.label ?? args.join(' '),
    displayCommand: args.join(' '),
    ...packageManagerInvocation(['exec', ...args]),
    ...options,
  }
}

/**
 * Spawn a package-manager mode (`run <script>` or `exec <bin> ...`) through the
 * manager that invoked this runner.
 *
 * Standalone package-manager binaries such as bun execute directly; node-backed
 * managers ship a JavaScript entry that must launch under node because Windows
 * cannot spawn their .cmd shims without a shell.
 */
function packageManagerInvocation(args: string[]): Pick<Gate, 'command' | 'args'> {
  const entrypoint = process.env.npm_execpath
  if (entrypoint === undefined || entrypoint === '') {
    throw new Error('run-gates: npm_execpath is unavailable; invoke the runner through a bun package script.')
  }
  const userAgent = process.env.npm_config_user_agent ?? ''
  if (!(/\.exe$/i.test(entrypoint) || userAgent.startsWith('bun/'))) {
    return { command: process.execPath, args: [entrypoint, ...args] }
  }
  if (args[0] !== 'exec') return { command: entrypoint, args }
  // `bun exec` resolves PATH entries only, never workspace binaries; exec gates
  // therefore target the workspace's installed launcher directly.
  const bin = args[1]
  if (bin === undefined) throw new Error('run-gates: exec gate is missing a binary name.')
  const launcher = resolve(root, 'node_modules', '.bin', process.platform === 'win32' ? `${bin}.exe` : bin)
  if (!existsSync(launcher)) {
    throw new Error(`run-gates: exec gate binary ${JSON.stringify(bin)} is not installed at ${launcher}.`)
  }
  return { command: launcher, args: args.slice(2) }
}
...
export function gatesForMode(selected: Mode): Gate[] {
  switch (selected) {
    ...
    case 'ci-lint-contracts-ready':
      return [
        lintGate(),
        scriptGate('duplication', 'duplication'),
      ]
    case 'ci-coverage':
    ...
    case 'check-all':
      return [
        scriptGate('runtime-closure', 'verify-runtime-closure', { label: 'runtime closure' }),
        scriptGate('cordis-config', 'verify-cordis-config', { label: 'Cordis config' }),
        scriptGate('client-domain-graph', 'verify-client-domain-graph', { label: 'client domain graph' }),
        scriptGate('test', 'test'),
        scriptGate('issue-management', 'test:issue-management', { label: 'Issue management policy' }),
        scriptGate('duplication', 'duplication'),
        snapshotGate(),
        scriptGate('build', 'build'),
        scriptGate('build:web', 'build:web'),
        ...
        scriptGate('module-graph', 'verify-module-graph', { label: 'module graph' }),
      ]
    case 'doc-sync':
...
function ciSharedStaticGates(): Gate[] {
  return [
    scriptGate('runtime-closure', 'verify-runtime-closure', { label: 'runtime closure' }),
    scriptGate('constraints', 'constraints'),
    scriptGate('dsh-package-licenses', 'verify-dsh-package-licenses', { label: 'DSH package licenses' }),
    scriptGate('package-invariants', 'verify-package-invariants', { label: 'package invariants' }),
    scriptGate('cordis-config', 'verify-cordis-config', { label: 'Cordis config' }),
    scriptGate('optional-dependency-imports', 'verify-optional-dependency-imports', {
      label: 'optional dependency imports',
    }),
    scriptGate('client-packages', 'verify-client-packages', { label: 'client packages' }),
    scriptGate('issue-management', 'test:issue-management', { label: 'Issue management policy' }),
  ]
}
...
function ciPrimaryGates(): Gate[] {
  return [
    ...ciSharedStaticGates(),
    typertContractsGate(),
    scriptGate('typecheck', 'typecheck:contracts-ready', { needs: ['typert-contracts'] }),
    ...
    scriptGate('publint', 'publint', { needs: ['build'] }),
    scriptGate('node-next-types', 'verify-node-next-types', {
...
function nodeCompatGates(): Gate[] {
  const typecheck = flagEnabled('DSH_NODE_COMPAT_SKIP_TYPECHECK')
    ? []
    : [scriptGate('typecheck', 'typecheck')]
...
function nodeCompatSmokeGates(options: { cliSmoke?: boolean } = {}): Gate[] {
  ...
  if (options.cliSmoke) {
    gates.push(
      execGate('cli-lazy-search-startup-smoke', [
        'vitest',
        'run',
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ciBuildGate(),
    scriptGate('node-compat', 'check:node-compat', {
      label: 'Node compatibility',
      env: { [CLIENT_BUILD_PROFILE_SELECTOR]: 'official' },
    }),
    scriptGate('publint', 'publint', { needs: builtTree }),
    builtPackageInvariantsGate(builtTree),
    scriptGate('lint-and-duplication', 'check:ci:lint:contracts-ready', {
      label: 'lint and duplication',
      needs: validatedBuild,
    }),
    snapshotGate(validatedBuild),
    webSnapshotGate(validatedBuild),
    scriptGate('doc-typecheck', 'doc-typecheck:contracts-ready', {
      needs: validatedBuild,
...
function webSnapshotGate(needs: string[]): Gate {
  ...
  if (workerRaw !== undefined && workerRaw !== '') {
    ...
    return scriptGate('web-snapshot', 'test:web:ci', {
  ...
  return scriptGate('web-snapshot', 'test:web:built', {
    label: 'web browser snapshot',
...
function ciWindowsBlockingGates(): Gate[] {
  return [
    ...
    scriptGate('windows-site', 'docs:build', { label: 'production site' }),
  ]
}
...
function coverageGates(): Gate[] {
  ...
  const instrumented = partitions === undefined
    ...
    : scriptGate('coverage', 'test:coverage:partitioned', {
      label: 'test:coverage',
  ...
  return [
    instrumented,
    execGate('coverage-exempt-heavy', [
      'vitest',
      'run',
...
function snapshotGate(needs: string[] = ['build']): Gate {
  return scriptGate('snapshot', 'test:snapshot', {
...
function builtPackageInvariantsGate(needs?: string[]): Gate {
  return scriptGate('built-package-invariants', 'verify-built-package-invariants', {
...
function hygieneLeafGates(options: { artifactNeeds?: string[] } = {}): Gate[] {
  ...
  return [
    scriptGate('rescope-vendor', 'rescope-vendor:check', { label: 'vendor rescope' }),
    scriptGate('knip', 'knip'),
    scriptGate('publint', 'publint', artifactOptions),
    scriptGate('constraints', 'constraints'),
    scriptGate('dsh-package-licenses', 'verify-dsh-package-licenses', { label: 'DSH package licenses' }),
    scriptGate('package-invariants', 'verify-package-invariants', { label: 'package invariants' }),
...
function docSyncLeafGates(options: {
  ...
  return [
    ...options.includeDocTypecheck === false
      ? []
      : [scriptGate('doc-typecheck', options.docTypecheckScript ?? 'doc-typecheck', docTypecheckOptions)],
    scriptGate('cordis-catalog', 'verify-cordis-catalog', { label: 'cordis catalog' }),
    scriptGate('client-catalog', 'verify-client-catalog', { label: 'client catalog' }),
    scriptGate('export-jsdoc', 'verify-export-jsdoc', { label: 'export jsdoc' }),
    scriptGate('tool-catalog', 'verify-tool-catalog', { label: 'tool catalog' }),
    scriptGate('config-catalog', 'verify-config-catalog', { label: 'config catalog' }),
    scriptGate('persistence-catalog', 'verify-persistence-catalog', { label: 'persistence catalog' }),
    scriptGate('doc-graphs', 'verify-doc-graphs', { label: 'doc graphs' }),
    scriptGate('scoped-events', 'verify-scoped-events', { label: 'scoped events' }),
    scriptGate('markdown-wrap', 'verify-md-wrap', { label: 'markdown wrap' }),
    scriptGate('markdown-links', 'verify-md-links', { label: 'markdown links' }),
    scriptGate('public-repository-links', 'verify-public-repository-links', { label: 'public repository links' }),
    scriptGate('doc-refs', 'verify-doc-refs', { label: 'doc refs' }),
    scriptGate('package-paths', 'verify-package-paths', { label: 'package paths' }),
    scriptGate('config-source-ownership', 'verify-config-source-ownership', { label: 'config source ownership' }),
    scriptGate('package-readme-model-experience', 'verify-package-readme-model-experience', { label: 'package README model experience' }),
    scriptGate('mermaid', 'verify-mermaid'),
    scriptGate('agent-note-classification', 'verify-agent-note-classification', { label: 'agent note classification' }),
    scriptGate('agent-note-format', 'verify-agent-note-format', { label: 'agent note format' }),
    scriptGate('archived-agent-notes', 'verify-archived-agent-notes', { label: 'archived agent notes' }),
    scriptGate('type-equivalence', 'verify-type-equiv', { label: 'type equivalence' }),
    scriptGate('skill-invocation-metadata', 'verify-skill-invocation-metadata', { label: 'skill invocation metadata' }),
    scriptGate('translation-prompt', 'verify-translation-prompt', { label: 'translation prompt' }),
    scriptGate('translation-pairing', 'verify-translation-pairing', { label: 'translation pairing' }),
    scriptGate('doc-budgets', 'verify-doc-budgets', { label: 'doc budgets' }),
    execGate('docs-site-projection', ['vitest', 'run', 'scripts/project-doc-site.spec.ts', 'scripts/verify-doc-site-fragments.spec.ts'], {
      label: 'documentation site checks',
    }),
    // Keep the VitePress build itself in one gate because projection rewrites website/.generated.
    scriptGate('docs-site-build', options.docsBuildScript ?? 'docs:build', { label: 'documentation build' }),
    scriptGate('package-readme-limitations', 'verify-package-readme-limitations', { label: 'package README limitations' }),
  ]
}

function builtBinSmokeGate(needs: string[] = ['build']): Gate {
  return execGate('built-bin-smoke', [
    'vitest',
    'run',
    '--config',
