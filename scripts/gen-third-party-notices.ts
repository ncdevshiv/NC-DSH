 ...
 * root patch list. License and repository metadata come from the installed
 * dependency tree, so the tree must be installed. `--check` verifies the
 * committed artifact. Tier policy and ownership live in
...
/** The `workspaces` member globs declared by the repository root manifest. */
function workspaceMembers(): string[] {
  const manifest = JSON.parse(readFileSync(resolve(root, 'package.json'), 'utf8')) as { workspaces?: unknown }
  const declared = manifest.workspaces
  if (!Array.isArray(declared) || declared.length === 0) {
    throw new Error('gen-third-party-notices: package.json declares no workspaces; the manifest set cannot be derived.')
  }
...
/**
 * Resolve one package's manifest inside an installed `node_modules` tree.
 * Bun hoists shared versions to the tree root and keeps version-conflicted
 * copies nested under their dependents, so a recursive scan over nested
 * `node_modules` directories finds whichever copy the closure installs.
 *
 * @param store - the `node_modules` directory to scan.
 * @param name - the external package name, exactly as `node_modules` spells it.
 * @returns the parsed manifest, or `undefined` when the tree does not contain
 *   the package.
 */
export function treeManifest(store: string, name: string): VirtualManifest | undefined {
  const direct = resolve(store, name, 'package.json')
  if (existsSync(direct)) {
    return JSON.parse(readFileSync(direct, 'utf8')) as VirtualManifest
  }
...
function installedManifest(name: string): VirtualManifest | undefined {
  ...
  // Bun keeps every installed version in its `.bun/node_modules` store with
  // hoisted symlinks pointing into it; a workspace-local link farm can expose
  // a dependency that the root tree does not name. All are backed by the root
  // workspace's lockfile.
  const stores = ['node_modules', 'node_modules/.bun/node_modules', 'native/landlock-run/node_modules']
  for (const store of stores) {
    manifest = treeManifest(resolve(root, store), name)
    if (manifest !== undefined) break
  }
  return manifest
}

/** License and repository URL for an installed external package, from an installed dependency tree. */
...
/** Patched external packages, from the repository root manifest. */
...
The complete npm transitive closure, including the Landlock launcher workspace, is recorded with exact pinned versions in [\`bun.lock\`](bun.lock) — inspect it with \`bun pm licenses\`. The Python closure is recorded separately in [\`python/sdk/uv.lock\`](python/sdk/uv.lock).
