...
function runScript(script: string, environment: NodeJS.ProcessEnv): void {
  ...
  // managers ship a JavaScript entry that must launch under node.
  const userAgent = process.env.npm_config_user_agent ?? ''
  const standaloneBinary = /\.exe$/i.test(packageManager) || userAgent.startsWith('bun/')
  const [command, ...args] = standaloneBinary
    ? [packageManager, 'run', script]
    : [process.execPath, packageManager, 'run', script]
