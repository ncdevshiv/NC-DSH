...
function withPnpmEntrypoint<T>(action: () => T): T {
  ...
  const previousAgent = process.env.npm_config_user_agent
  process.env.npm_execpath = '/private/pnpm.cjs'
  process.env.npm_config_user_agent = 'pnpm/10.18.3 npm/? node/v24.19.0 linux x64'
  try {
    return action()
  } finally {
    if (previous === undefined) Reflect.deleteProperty(process.env, 'npm_execpath')
    else process.env.npm_execpath = previous
    if (previousAgent === undefined) Reflect.deleteProperty(process.env, 'npm_config_user_agent')
    else process.env.npm_config_user_agent = previousAgent
  }
}

/** Simulate a standalone-binary package manager (bun) for one action. */
function withBunEntrypoint<T>(action: () => T): T {
  return withEnv('npm_execpath', '/home/runner/.bun/bin/bun', () =>
    withEnv('npm_config_user_agent', 'bun/1.4.0 npm/? node/v24.19.0 linux x64', action))
}
...
describe('package manager invocation', () => {
  ...
  it('routes exec gates through the workspace-installed launcher under bun', () => {
    const subject = withBunEntrypoint(() =>
      gatesForMode('node-compat').find(item => item.id === 'vitest-jsdom-smoke'))

    expect(subject?.command).toBe(resolve(
      import.meta.dirname,
      '..',
      'node_modules',
      '.bin',
      process.platform === 'win32' ? 'vitest.exe' : 'vitest',
    ))
    expect(subject?.args).toEqual(['run', 'scripts/vitest-environment.compat.spec.ts'])
  })
})
