...
// The jsdom component suites (the *.client.spec.* convention) run under
// vmForks: @adobe/react-spectrum's ESM dist carries side-effect .css imports,
// and only the vm pools intercept external-module CSS — under plain forks a
// native Node import of those files throws ERR_UNKNOWN_FILE_EXTENSION. Every
// non-DOM suite stays on forks for the Node-stability reason below.
const clientDomIncludes = [
  'packages/client/*/tests/**/*.client.spec.{ts,tsx}',
  'apps/web/tests/**/*.client.spec.{ts,tsx}',
]

// Node-half behaviors named .client.spec: they boot without a DOM by
// redefining document/window, which VM-context globals forbid.
const clientDomExcludes = [
  'packages/client/modules/tests/*.client.spec.ts',
  'packages/client/locale/tests/*.client.spec.ts',
]
...
export default defineConfig({
  ...
  test: {
    ...
    // One coverage invocation aggregates all three projects. Suites fork for
